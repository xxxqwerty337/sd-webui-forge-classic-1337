...
class Script(scripts.Script):
    ...
    def ui(self, is_img2img):
        ...
        return [use_random_seed, batch_count, run_adetailer, use_hires_fix, hires_denoise, hires_upscale]

    def run(self, p, use_random_seed, batch_count, run_adetailer, use_hires_fix, hires_denoise, hires_upscale, *args):
        import warnings
        import logging
        ...
        # Save & apply the global options tied to the hires preset (RNG, Emphasis).
        # These are restored at the end of the run.
        original_global_opts = {}
        if use_hires_fix:
            for opt_name, opt_value in HIRES_GLOBAL_OPTS.items():
                if hasattr(shared.opts, opt_name):
                    original_global_opts[opt_name] = getattr(shared.opts, opt_name)
            ...
            for batch_num in range(batch_count):
                ...
                for idx, (image_key, metadata) in enumerate(valid_entries):
                    ...
                    if use_hires_fix:
                        ...
                        # Use the scale ratio, not a fixed target resolution.
                        proc.hr_resize_x = 0
                        proc.hr_resize_y = 0
                        proc.hr_upscaler = HIRES_FIX_SETTINGS["hr_upscaler"]
                        proc.hr_second_pass_steps = HIRES_FIX_SETTINGS["hr_second_pass_steps"]
                        # Neo-specific hires attributes. setattr always succeeds; whether a
                        # given attribute is actually consumed depends on the WebUI Neo build.
                        for attr in ("hr_cfg", "hr_shift", "hr_additional_modules"):
                            setattr(proc, attr, HIRES_FIX_SETTINGS[attr])
                        # Surface the preset in the saved PNG infotext for verification.
        ...
        # Restore any global options changed for the hires preset.
        for opt_name, opt_value in original_global_opts.items():
            setattr(shared.opts, opt_name, opt_value)
